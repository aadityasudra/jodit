		...
		for (i = length - 1; i >= 0; --i) {
			...
			if (startElm) {
				if (info[i].collapsed) {
					...
					if (info[i].endId) {
						const endElm: HTMLElement | null =
							this.doc.getElementById(info[i].endId as string);
						if (endElm) {
			...
			const forEvery = (current: Node): void => {
				...
				if (current.nodeName === 'LI') {
					if (current.firstChild) {
						...
						const currentB =
							this.jodit.create.inside.text(INVISIBLE_SPACE);
		...
		if (!this.isCollapsed()) {
			...
			$$('font[size="7"]', this.area).forEach((font: HTMLElement) => {
				if (
					...
					const leftFragment: DocumentFragment =
