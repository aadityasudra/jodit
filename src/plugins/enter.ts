	...
	editor.events.on('keydown', (event: KeyboardEvent): false | void => {
		if (event.which === consts.KEY_ENTER) {
			...
			let currentBox: HTMLElement | false = current
				...
				: false;
			const isLi: boolean = currentBox && currentBox.nodeName === 'LI';
			...
			if (
				...
				const br: HTMLBRElement = editor.create.inside.element('br');
			...
			if (
				...
				if (Dom.isEmpty(currentBox)) {
					const helper_node: HTMLBRElement =
						editor.editorDocument.createElement('br');
