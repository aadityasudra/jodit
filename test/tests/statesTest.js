	...
	describe('ReadOnly', function () {
		describe('Set readonly mode in options', function () {
			describe('Readonly', function () {
				it('Should deny edit content in simple source editor', function () {
					...
					expect(true).to.be.equal(
						editor.__plugins.source.mirror.hasAttribute('readonly')
					);
				});
			});
			...
			it('Should disable all toolbar buttons besides source, print, about, fullsize', function () {
				...
				const buttons = [].slice.call(
					editor.container.querySelectorAll('.jodit_toolbar_btn')
				);
				buttons.forEach(function (btn) {
					if (
						...
						expect(true).to.be.equal(
							btn.classList.contains('jodit_disabled')
						);
						...
						events: {
							aceInited: function (editor) {
								expect(null).to.be.not.equal(
									editor.__plugins.source.aceEditor
								);
								expect(true).to.be.equal(
									editor.__plugins.source.aceEditor.getReadOnly()
								);
								done();
							}
						}
		...
		describe('Set readonly mode by source element attribute', function () {
			it('Should work like by options', function () {
				...
				expect(editor.editor.hasAttribute('contenteditable')).to.be
					.false;
					...
					events: {
						aceInited: function (editor) {
							...
							expect(false).to.be.equal(
								editor.__plugins.source.mirror.hasAttribute(
									'readonly'
								)
							);
							expect(false).to.be.equal(
	...
	describe('Disabled', function () {
		describe('Set disabled mode in options', function () {
			it("Should enable readonly mode too and editor's container should have jodit_disabled class", function () {
				...
				expect(editor.container.classList.contains('jodit_disabled')).to
					.be.true;
				expect(editor.editor.hasAttribute('contenteditable')).to.be
					.false;
				expect(editor.getReadOnly()).to.be.true;
