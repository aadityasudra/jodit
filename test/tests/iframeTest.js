			...
			const editor = new Jodit(appendTestArea(), {
				...
				events: {
					afterConstructor: function (editor) {
						expect(editor.ownerDocument).to.be.not.equal(
				...
				const editor = new Jodit(appendTestArea(), {
					...
					events: {
						afterConstructor: function (editor) {
							...
							expect('test test stop').to.be.equal(
							...
							expect(
								'<strong>test test stop</strong>'
							).to.be.equal(editor.editorDocument.body.innerHTML);
							done();
						}
					}
				});
			...
			const editor = new Jodit(area, {
				...
				events: {
					afterConstructor: function (jodit) {
						...
						).to.be.equal('435'); // loaded from index.html
						expect(
							Jodit.modules.Helpers.trim(jodit.value)
						).to.be.equal('test 435'); // loaded from index.html
						done();
					},
